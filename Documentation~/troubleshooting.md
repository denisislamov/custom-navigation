...
| --- | --- | --- | --- |
| Compile сообщает, что `Jitter2.Core.dll` из `precompiledReferences` отсутствует | Custom Navigation установлен раньше separate Jitter prerequisite | Найдите `Jitter2.Core.dll` под `Assets`; проверьте порядок установки | Установите approved `jitter-v2.8.9-datasakura.1-rc.1` release в project-owned plugin folder, затем импортируйте Custom Navigation |
| Preflight сообщает missing/duplicate/hash/f64 Jitter | Нет DLL, есть две копии, DLL не из approved release или собрана с `USE_DOUBLE_PRECISION` | Проверьте inventory и SHA-256 `944666bbe73dfce5ffc5bfb18569fb0004f50e767dcbb8b471dde15242023ca6` | Оставьте ровно одну approved f32 DLL; не берите её транзитивно из Baker/package cache |
...
| `SchemaVersion mismatch ... Schema 1 ... re-baked and re-exported` | Artifact создан до JMP migration и не содержит compatibility identity | Сравните `Schema` в Build Details с `NavigationArtifactLoader.SupportedSchemaVersion` (`2`) | Не редактируйте manifest и не переносите старый hash: выполните новый `Build for Client`, затем Upload/Export |
| Wire response/request отклонён с `PrecisionMismatch`, `CanonicalJitterMismatch`, `DeterministicMathMismatch` или `FingerprintAlgorithmMismatch` | Client/server собраны с разной canonical identity | Сравните `/health`, manifest и protocol fields | Установите exact Jitter release и Custom Navigation version на обеих сторонах; re-bake/re-export |
| Route request отклонён `ArtifactHash mismatch ... before DotRecast` | Client не передал hash либо server загрузил другой artifact | Сравните полный client/server SHA-256 и `levelId` | Передавайте exact non-empty `clientArtifactHash`; доставьте schema-2 artifact на server, silent fallback запрещён |
